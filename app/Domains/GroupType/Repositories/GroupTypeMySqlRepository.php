...
class GroupTypeMySqlRepository implements GroupTypeRepositoryInterface
{
    ...
    public function list()
    {
        ...
        $query = $this->groupType::when(request()->sort_by, function ($q) {
...
//            . date("Y-m-d-his")
    ...
    public function update(string $id, $request): bool
    {
        ...
        if($request->code){
            if($request->code>9||$request->code<0){
                return false;
            }
            if($groupType->children&&count($groupType->children)>0){
                return false;
            }

            $previous = $this->groupType::where('code',$request->code)->first();
            if($previous){
                if($previous->children&&count($previous->children)>0){
                    return false;
                }
                $previous->code = $code;
                $previous->save();
